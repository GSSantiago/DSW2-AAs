...
app.put('/family/:id', (req, res) => {
  ...
  if (familyIndex === -1) return res.status(404).json({ error: 'Family not found' });
...
app.get('/family/:familyId/entries', (req, res) => {
  ...
  const usersInFamily = db.users?.filter(user => user.currentFamilyId === familyId) || [];
  
  // Extract user IDs
  const userIds = usersInFamily.map(user => user.id);
  
  // Get all entries from these users
  const entries = db.entries?.filter(entry => userIds.includes(entry.userId)) || [];
  ...
  // Sort by date descending (newest first)
  const sortedEntries = entriesWithUserInfo.sort((a, b) => new Date(b.date) - new Date(a.date));
...
app.get('/family/:familyId/entries/summary', (req, res) => {
  ...
  const summary = {
    ...
    totalIncome: entries.filter(e => e.type === 'income').reduce((sum, e) => sum + e.value, 0),
    totalExpenses: entries.filter(e => e.type === 'expense').reduce((sum, e) => sum + e.value, 0),
    ...
    entriesByUser: userIds.map(userId => {
      ...
      return {
        ...
        totalIncome: userEntries.filter(e => e.type === 'income').reduce((sum, e) => sum + e.value, 0),
        totalExpenses: userEntries.filter(e => e.type === 'expense').reduce((sum, e) => sum + e.value, 0),
...
app.listen(PORT, () => {
  ...
  console.log(`  GET    /users/:userId/entries              # Get all entries for a specific user`);
  console.log(`  GET    /users/:userId/entries/summary     # Get entries summary for a specific user`);
  console.log(`  GET    /family/:familyId/entries          # Get all entries from users in a family`);
  console.log(`  GET    /family/:familyId/entries/summary # Get entries summary for a family`);
